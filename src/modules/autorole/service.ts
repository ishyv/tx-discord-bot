...
import { AutoRoleGrantsStore, AutoRoleRulesStore, AutoRoleTalliesStore, autoroleKeys } from "./data/store";
import { enqueueRoleGrant, enqueueRoleRevoke } from "./engine/roleOps";
import { clearPresenceForMessage, deleteTalliesForMessage, getGuildRules, setTally, deleteTally, removeRule as removeRuleFromCache, upsertRule as upsertRuleInCache } from "./cache";
import { isFeatureEnabled, Features } from "@/modules/features";
import type { ReactionTallyKey, ReactionTallySnapshot, ReactionPresenceKey, CreateAutoRoleRuleInput } from "./domain/types";
...
export class AutoroleService {
    ...
    static async grantByRule({
        ...
        const grantType: AutoRoleGrantType = isLiveRule(rule.durationMs) ? "LIVE" : "TIMED";
        const grantId = autoroleKeys.grant(rule.guildId, userId, rule.roleId, rule.name, grantType);
     ...
     */
    static async createRule(input: CreateAutoRoleRuleInput): Promise<AutoRoleRule> {
     ...
     */
    static async toggleRule(guildId: string, name: string, enabled: boolean): Promise<AutoRoleRule | null> {
        const id = autoroleKeys.rule(guildId, name);
