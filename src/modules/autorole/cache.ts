...
export function upsertRule(rule: AutoRoleRule): void {
  ...
  for (const [, list] of cache.reactSpecific) filterInPlace(list, (r) => r.name !== rule.name);
  for (const [, list] of cache.reactedByEmoji) filterInPlace(list, (r) => r.name !== rule.name);
...
function bucketRule(cache: GuildRuleCache, rule: AutoRoleRule) {
  ...
  switch (rule.trigger.type) {
    ...
    case "REPUTATION_THRESHOLD":
      ...
      cache.repThresholds.sort((a, b) =>
        (a.trigger.type === "REPUTATION_THRESHOLD" ? a.trigger.args.minRep : 0) -
        (b.trigger.type === "REPUTATION_THRESHOLD" ? b.trigger.args.minRep : 0)
      );
      break;
    case "ANTIQUITY_THRESHOLD":
      cache.antiquityThresholds.push(rule);
      cache.antiquityThresholds.sort((a, b) =>
        (a.trigger.type === "ANTIQUITY_THRESHOLD" ? a.trigger.args.durationMs : 0) -
        (b.trigger.type === "ANTIQUITY_THRESHOLD" ? b.trigger.args.durationMs : 0)
      );
      break;
...
export function clearPresenceForMessage(guildId: string, messageId: string): ReactionPresenceKey[] {
  ...
  for (const value of Array.from(presenceKeys)) {
    if (value.startsWith(prefix)) {
      ...
      if (emojiKey && userId) removed.push({ guildId, messageId, emojiKey, userId });
    }
  }
  return removed;
}
...
export function setTally(snapshot: ReactionTallySnapshot): void {
  tallySnapshots.set(makeTallyKey(snapshot.guildId, snapshot.messageId, snapshot.emojiKey), snapshot);
}

export function getTally(key: ReactionTallyKey): ReactionTallySnapshot | null {
  return tallySnapshots.get(makeTallyKey(key.guildId, key.messageId, key.emojiKey)) ?? null;
}
...
export function deleteTalliesForMessage(guildId: string, messageId: string): number {
  let removed = 0;
  const prefix = `${guildId}:${messageId}:`;
