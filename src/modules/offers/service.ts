...
> {
  if (!approvedChannelId)
    return OkResult({ publishedChannelId: null, publishedMessageId: null });
  try {
...
export async function createOfferForReview(
  ...
  const existingResult = await findActiveByAuthor(
    params.guildId,
    params.authorId,
  );
...
export async function approveOffer(
  ...
  const { approvedChannelId, generalLogsId } = await resolveChannels(
    client,
    updated.guildId,
  );
  ...
  const publishData = publishResult.unwrapOr({
    publishedChannelId: null,
    publishedMessageId: null,
  });
