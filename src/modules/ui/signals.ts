...
const isSignalKey = <T extends Record<string, unknown>>(
  ...
  typeof prop === "string" &&
  Object.prototype.hasOwnProperty.call(signals, prop);
...
export function createSignals<T extends Record<string, unknown>>(
  ...
  return Object.fromEntries(
    Object.keys(initial).map((key) => [
      key,
      new Signal((initial as any)[key], onChange),
    ]),
  ) as SignalMap<T>;
}
...
export function createStateProxy<T extends Record<string, unknown>>(
  ...
  const handler: ProxyHandler<typeof base> = {
    ...
    set(target, prop, value) {
      if (prop === "$") {
        console.warn("createStateProxy: attempted to overwrite '$'; ignoring.");
        return true;
      }
      if (isSignalKey(signals, prop)) {
        const key = prop as keyof T;
    ...
    has(target, prop) {
      return (
        prop === "$" || isSignalKey(signals, prop) || Reflect.has(target, prop)
      );
    },
    ownKeys(target) {
      const keys = new Set<string | symbol>(
        Reflect.ownKeys(target) as Array<string | symbol>,
      );
