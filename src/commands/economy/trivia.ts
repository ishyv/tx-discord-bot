...
export default class TriviaCommand extends Command {
	async run(ctx: GuildCommandContext<typeof triviaOptions>) {
		...
		if (
			...
			await ctx.write({
				embeds: [buildErrorEmbed("Trivia is disabled on this server.")],
				flags: MessageFlags.Ephemeral,
			});
			return;
		}
		...
		const existingSession = await minigameService.getTriviaSession(
			userId,
			guildId,
		);
		if (existingSession) {
			...
			if (new Date() > existingSession.expiresAt) {
				...
				await this.showExistingQuestion(
					ctx,
					existingSession.questionId,
