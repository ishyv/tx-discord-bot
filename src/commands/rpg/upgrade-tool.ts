...
@Declare({
  name: "upgrade-tool",
  description: "Upgrade a tool to the next tier with materials and money",
  contexts: ["Guild"],
...
@Options(options)
export default class UpgradeToolCommand extends Command {
  async run(ctx: GuildCommandContext<typeof options>) {
    ...
    if (useEquipped) {
      ...
      if (!equippedResult) {
        await ctx.editOrReply({
          content: "❌ You don't have a tool equipped! Use `/rpg equip` to equip one, or provide an instance ID.",
        });
        return;
      }
  ...
  private async showUpgradeHelp(
    ...
    content += "\n**Usage:**\n";
    content += "• `/upgrade-tool equipped:true` - Upgrade your equipped tool\n";
    content += "• `/upgrade-tool instance:<id>` - Upgrade a specific tool by instance ID\n";
