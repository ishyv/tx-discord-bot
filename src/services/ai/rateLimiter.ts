...
export class AiRateLimiter {
    ...
    async consume(guildId: string, userId: string, maxUses: number, windowSeconds: number): Promise<ConsumeResult> {
        ...
        if (existing && now < existing.resetAt) {
            if (existing.count >= maxUses) {
                console.log(`[ai-rate-limit] blocked guild=${guildId} user=${userId} count=${existing.count} max=${maxUses}`);
        ...
        console.log(`[ai-rate-limit] consume(new) guild=${guildId} user=${userId} remaining=${maxUses - count}`);
