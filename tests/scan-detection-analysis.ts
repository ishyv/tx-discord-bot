...
function analyzeScamFilters(): FilterAnalysis {
  ...
  scamFilterList.forEach(filter => {
    ...
    } else if (source.includes('roblox') || source.includes('solara') || source.includes('executor')) {
      categories.gaming++;
    } else if (source.includes('nitro') || source.includes('discord')) {
      categories.discord_nitro++;
    } else if (source.includes('free') || source.includes('bonus') || source.includes('code')) {
      categories.free_bonus++;
    } else if (source.includes('.xyz') || source.includes('.click') || source.includes('.info')) {
...
export function performScanDetectionAnalysis(): void {
  ...
  systemAnalysis.triggerConditions.forEach(condition => console.log(condition));
  console.log('');
  ...
  console.log('\nCommon trigger words:', filterAnalysis.commonTriggers.join(', '));
  console.log('\nFalse positive risks:');
  filterAnalysis.falsePositiveRisks.forEach(risk => console.log(`  - ${risk}`));
  console.log('');
  ...
  console.log('\nShould be detected:');
  testScenarios.successfulDetection.forEach(scenario => console.log(`  ✓ ${scenario}`));
  console.log('\nMay be false positives:');
  testScenarios.likelyFalsePositives.forEach(scenario => console.log(`  ⚠ ${scenario}`));
  console.log('\nMay be false negatives:');
  testScenarios.likelyFalseNegatives.forEach(scenario => console.log(`  ✗ ${scenario}`));
  console.log('\nEdge cases to test:');
