...
interface TestCase {
  id: string;
  category: 'clean_scan' | 'angled_document' | 'screenshot' | 'ui_with_text' | 'no_text' | 'edge_case';
  description: string;
  imagePath?: string;
...
async function runSingleTest(testCase: TestCase): Promise<TestResult> {
  ...
  try {
    ...
    const matchingFilters = scamFilterList.filter(filter => filter.test(normalizedText));
...
async function analyzeScamFilters(): Promise<void> {
  ...
  scamFilterList.forEach(filter => {
    ...
    if (source.includes('crypto') || source.includes('eth') || source.includes('btc')) {
      filterCategories.crypto++;
    } else if (source.includes('nitro') || source.includes('discord')) {
      filterCategories.nitro++;
    } else if (source.includes('bonus') || source.includes('free') || source.includes('code')) {
...
async function runEvaluation(): Promise<EvaluationSummary> {
  ...
  for (const testCase of TEST_CASES) {
    ...
    console.log(`Result: ${result.detectedAsScam ? 'DETECTED' : 'NOT DETECTED'}`);
  ...
  const summary: EvaluationSummary = {
    ...
    successfulDetections: results.filter(r => 
      r.detectedAsScam === r.testCase.expectedDetection
    ).length,
    falsePositives: results.filter(r => 
      r.detectedAsScam && !r.testCase.expectedDetection
    ).length,
    falseNegatives: results.filter(r => 
      !r.detectedAsScam && r.testCase.expectedDetection
    ).length,
  ...
  categories.forEach(category => {
    ...
    summary.categoryBreakdown[category] = {
      ...
      correct: categoryResults.filter(r => r.detectedAsScam === r.testCase.expectedDetection).length,
      falsePositives: categoryResults.filter(r => r.detectedAsScam && !r.testCase.expectedDetection).length,
      falseNegatives: categoryResults.filter(r => !r.detectedAsScam && r.testCase.expectedDetection).length
    };
  });
...
export async function evaluateScanDetection(): Promise<void> {
  try {
    ...
    console.log(`Successful detections: ${summary.successfulDetections} (${(summary.successfulDetections/summary.totalTests*100).toFixed(1)}%)`);
    console.log(`False positives: ${summary.falsePositives}`);
    console.log(`False negatives: ${summary.falseNegatives}`);
    console.log(`Average processing time: ${summary.averageProcessingTime.toFixed(1)}ms`);
    
    console.log('\n=== CATEGORY BREAKDOWN ===');
    Object.entries(summary.categoryBreakdown).forEach(([category, stats]) => {
      const accuracy = (stats.correct / stats.total * 100).toFixed(1);
      console.log(`${category}: ${stats.correct}/${stats.total} (${accuracy}%) - FP: ${stats.falsePositives}, FN: ${stats.falseNegatives}`);
    });
